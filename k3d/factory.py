...
def multi_mip(
    ...
    Parameters
    ----------
    volume_list: list[ndarray]
        A list of 3D arrays of `float`
    color_map_list: list[list], optional
        A list of lists of `float` quadruplets (attribute value, R, G, B), sorted by attribute value, by default None.
        The first quadruplet should have value 0.0, the last 1.0; R, G, B are RGB color components in the range 0.0 to 1.0.
    opacity_function_list: list[list], optional
        A list of lists of `float` tuples (attribute value, opacity), sorted by attribute value, by default None.
        The first tuples should have value 0.0, the last 1.0; opacity is in the range 0.0 to 1.0.
    color_range_list: list[list], optional
        A list of [min_value, max_value] pair determining the levels of color attribute mapped
        to 0 and 1 in the color map respectively.
    samples: float, optional
        Number of iteration per 1 unit of space, by default 512.0.
    gradient_step : float, optional
        Gradient light step, by default 0.005.
    alpha_blending: bool, optional
        Whether alpha blending is enabled, by default False
    interpolation : bool, optional
        Interpolate volume raycasting data, by default True.
    mask: `array_like`.
        3D array of `int` in range (0, 255).
    ...
    **kwargs
    ...
    MultiMIP
        MultiMIP Drawable.
    """
    if color_map_list is None:
        color_map_list = [default_colormap]
    ...
    if opacity_function_list is None:
        ...
        for color_map in color_map_list:
            opacity_function_list.append([
                np.min(color_map[::4]), 0.0, 
                np.max(color_map[::4]), 1.0
            ])
    ...
    return process_transform_arguments(
        MultiMIP(
            ...
            color_map_list=color_map_list,
            opacity_function_list=opacity_function_list,
            color_range_list=color_range_list,
            samples=samples,
            gradient_step=gradient_step,
